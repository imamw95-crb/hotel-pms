...
// Daftar kamar valid
$VALID_ROOMS = [
    '0101', '0102', '0103', '0105', '0106', '0107', '0108', '0109', '0110', '0111', '0112',
...
switch ($action) {
    case 'checkin':
        ...
        // Validasi
        $errors = [];
        if (empty($room)) $errors[] = 'Room number required';
        if (empty($name)) $errors[] = 'Guest name required';
        if (empty($checkin) || !preg_match('/^\d{12}$/', $checkin)) $errors[] = 'Valid checkin date required (YYYYMMDDHHMM)';
        if (empty($checkout) || !preg_match('/^\d{12}$/', $checkout)) $errors[] = 'Valid checkout date required (YYYYMMDDHHMM)';
