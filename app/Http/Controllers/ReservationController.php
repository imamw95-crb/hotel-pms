...
class ReservationController extends Controller
{
    ...
    public function show(Reservation $reservation)
    {
        ...
        // ΓöÇΓöÇΓöÇ Group Booking Info ΓöÇΓöÇΓöÇ
    ...
    public function addPayment(Request $request, Reservation $reservation)
    {
        ...
        try {
            ...
            if ($hotelAmount > 0) {
                ...
                // ≡ƒöÉ OTS: Timestamp receipt pembayaran (DP/Pelunasan/Tambahan)
    ...
    public function groupPayment(Request $request, string $bookingGroupId)
    {
        ...
        try {
            ...
            foreach ($reservations as $res) {
                ...
                // ≡ƒöÉ OTS: Timestamp receipt pelunasan
    ...
    public function updateDates(Request $request, Reservation $reservation)
    {
        ...
        try {
            ...
            Transaction::create([
                ...
                'notes' => 'Ubah tanggal: check-in '.$oldCheckIn->format('d/m/Y').' ΓåÆ '.$newCheckIn->format('d/m/Y').', check-out '.$oldCheckOut->format('d/m/Y').' ΓåÆ '.$newCheckOut->format('d/m/Y').'. Total baru: Rp '.number_format($newTotal, 0, ',', '.'),
                'created_by' => auth()->id(),
            ]);
