...
class AiActionService
{
        ...
        if ($guests->isNotEmpty()) {
            ...
            if ($reservations->count() > 1) {
                ...
                return [
                    'reservation' => null,
                    'multiple' => true,
                    'matches' => $reservations->take(5)->map(fn ($r) => "`{$r->reservation_number}` — {$r->guest->guest_name} (Kamar {$r->room->room_number}, {$r->status})"
                    )->implode("\n"),
                ];
            }
        }
    ...
    public function actionCheckout(array $data): array
    {
        ...
        try {
            return DB::transaction(function () use ($reservation, $amount, $paymentMethod) {
                ...
                if ($amount > 0) {
                    $msg .= ' | Bayar '.$fmt($amount).' ('.$paymentMethod.')';
                }
    ...
    public function actionChangeRoom(array $data): array
    {
        ...
        if (! $newRoom) {
            return ['success' => false, 'message' => "Kamar {$newRoomNumber} tidak ditemukan."];
        }
        if ($newRoom->id === $reservation->room_id) {
            return ['success' => false, 'message' => "Sudah di kamar {$newRoomNumber}."];
        }
        if ($newRoom->status !== 'available') {
            return ['success' => false, 'message' => "Kamar {$newRoomNumber} sedang {$newRoom->status}."];
        }
    ...
    public function actionDepositCreate(array $data): array
    {
        $query = $data['query'] ?? '';
        if (empty($query)) {
            return ['success' => false, 'message' => 'Sebutkan nama tamu.'];
        }
        ...
        if (! $guest) {
            ...
            if ($guests->count() === 1) {
                ...
                return ['success' => true, 'message' => 'Beberapa tamu ditemukan: '.$guests->take(5)->pluck('guest_name')->implode(', ').'. Sebutkan nama lengkap.'];
            } else {
                return ['success' => false, 'message' => "Tamu '{$query}' tidak ditemukan."];
            }
        }
    ...
    public function actionDepositReturn(array $data): array
    {
        ...
        if (! $deposit && $query) {
            ...
            if (! $deposit) {
                ...
                if ($guest) {
                    $deposit = Deposit::where('guest_id', $guest->id)->where('status', 'active')->latest()->with('guest')->first();
                }
            }
        }
        if (! $deposit) {
            return ['success' => false, 'message' => 'Tidak ada deposit aktif.'];
        }
    ...
    public function actionHousekeeping(array $data): array
    {
        ...
        if (empty($roomNumber)) {
            return ['success' => false, 'message' => 'Sebutkan nomor kamar.'];
        }
        $room = Room::where('room_number', $roomNumber)->first();
        if (! $room) {
            return ['success' => false, 'message' => "Kamar {$roomNumber} tidak ditemukan."];
        }
    ...
    public function actionExtendStay(array $data): array
    {
        $query = $data['query'] ?? '';
        if (empty($query)) {
            return ['success' => false, 'message' => 'Sebutkan nama/reservasi untuk perpanjangan.'];
        }

        $resolved = $this->resolveReservation($query);
        if ($resolved['multiple']) {
            return ['success' => true, 'message' => "Ditemukan:\n{$resolved['matches']}\nSebutkan nomor reservasi."];
        }
        ...
        if ($newCO->lte($currentCO)) {
            return ['success' => false, 'message' => "Harus setelah {$currentCO->format('d M')}."];
        }

        $room = $reservation->room;
        if ($room && ! $room->isAvailable($reservation->check_in, $newCO, $reservation->id)) {
            return ['success' => false, 'message' => "Kamar {$room->room_number} tidak tersedia untuk tanggal tsb."];
        }
        ...
        try {
            ...
            if ($additionalAmount > 0) {
                $reservation->increment('total_amount', $additionalAmount);
            }
            $fmt = fn ($v) => 'Rp '.number_format($v, 0, ',', '.');
    ...
    public function actionUpdateRate(array $data): array
    {
        ...
        if (empty($query) || $newRate <= 0) {
            return ['success' => false, 'message' => 'Sebutkan tamu dan tarif baru.'];
        }
        ...
        if (in_array($reservation->status, ['checked_out', 'cancelled'])) {
            return ['success' => false, 'message' => "{$reservation->status_label}, tidak bisa diupdate."];
        }
    ...
    public function actionToggleBreakfast(array $data): array
    {
        $query = $data['query'] ?? '';
        if (empty($query)) {
            return ['success' => false, 'message' => 'Sebutkan nama/reservasi.'];
        }
        ...
        if (in_array($reservation->status, ['checked_out', 'cancelled'])) {
            return ['success' => false, 'message' => "{$reservation->status_label}."];
        }
