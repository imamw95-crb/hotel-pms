 ...
 * Endpoint: http://100.98.230.92/bridge_api.php
 */
header('Content-Type: application/json');

define('MHS_SERVER_IP', '100.98.230.92');
        ...
        if ($ok && $resp) {
            ...
            foreach ($parts as $part) {
                if (strlen($part) > 1) {
                    ...
                    if ($fieldId == 'R') {
                        $cardData['room'] = $fieldValue;
                    }
                    if ($fieldId == 'N') {
                        $cardData['name'] = $fieldValue;
                    }
                    if ($fieldId == 'D') {
                        $cardData['checkin'] = $fieldValue;
                    }
                    if ($fieldId == 'O') {
                        $cardData['checkout'] = $fieldValue;
                    }
                }
            }
        }
