/**
 * ============================================
 * HOTEL PMS — Booking Create JS
 * Handles booking create page: availability check,
...
(function() {
        ...
        xhr.onload = function() {
            if (xhr.status >= 200 && xhr.status < 300) {
                try {
                    ...
                    console.error('BookingCreate: Parse error', e);
                }
            }
        };
    ...
    // ── Initialize ──
    function init() {
        // Get API URL from meta or route
        ...
        // Fallback: try to find route from window or data attributes
        if (!apiUrl) {
            var dashboardEl = document.getElementById('roomsDashboard');
            if (dashboardEl) {
                apiUrl = dashboardEl.dataset.roomsApi ? dashboardEl.dataset.roomsApi.replace('/api/rooms', '/booking/check-availability') : '';
            }
        }
        ...
        var preSelectedRoomId = window._preSelectedRoomId || '';
        var preSelectedRoomNumber = window._preSelectedRoomNumber || '';
        if (preSelectedRoomId && roomInfo) {
            roomInfo.textContent = 'Kamar ' + preSelectedRoomNumber + ' dipilih dari Dashboard. Pilih tanggal untuk konfirmasi ketersediaan.';
