...
@section('title', 'Pendapatan Resto')
@section('header', 'Pendapatan Resto')
...
<div class="max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
            <div class="text-xs uppercase text-gray-500 font-bold">Pendapatan Hari Ini</div>
        ...
        <div class="bg-white rounded-lg shadow p-4 border-l-4 border-purple-500">
            ...
            <div class="text-2xl font-bold text-purple-700 mt-1">{{ $transactions->total() }}</div>
        </div>
    </div>
        ...
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" value="{{ $search }}"
                       placeholder="No. transaksi, meja, atau nama tamu..."
            ...
            @if($dateFrom || $dateTo || $search)
                <a href="{{ route('resto.index') }}" class="text-gray-500 hover:text-gray-700 text-sm px-3 py-2">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-800 text-white">
                    <th class="text-left p-3 font-semibold">No. Transaksi</th>
                    <th class="text-left p-3 font-semibold">Tanggal</th>
                    <th class="text-left p-3 font-semibold">Tamu</th>
                    <th class="text-center p-3 font-semibold">Meja</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                    <td class="p-3 font-mono font-bold text-blue-600">{{ $txn->transaction_number }}</td>
                    <td class="p-3 text-gray-600">{{ $txn->created_at->format('d/m/Y H:i') }}</td>
                    <td class="p-3">
                        <div class="font-medium">{{ $txn->guest->guest_name ?? '-' }}</div>
                        @if($txn->reservation)
                            <div class="text-xs text-gray-400">Kamar {{ $txn->reservation->room->room_number ?? '-' }}</div>
                        @endif
                    </td>
                    <td class="p-3 text-center">{{ $txn->table_number ?? '-' }}</td>
                    <td class="p-3">
                        @foreach(array_slice($txn->items, 0, 2) as $item)
                            <div class="text-xs">{{ $item['name'] }} ×{{ $item['qty'] }}</div>
                        @endforeach
                        @if(count($txn->items) > 2)
                            <div class="text-xs text-gray-400">+{{ count($txn->items) - 2 }} item lainnya</div>
                        @endif
                    </td>
                    <td class="p-3 text-right font-bold">Rp {{ number_format($txn->total_amount, 0, ',', '.') }}</td>
