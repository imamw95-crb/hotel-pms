{{-- Promo Price Edit Modal — no layout, pure HTML for AJAX modal --}}
<div class="p-6">
    ...
    <form method="POST" action="{{ route('promo-prices.update', $roomTypeDatePrice) }}" data-ajax="true" data-refresh="true">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="number" name="price" value="{{ old('price', $roomTypeDatePrice->price) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required min="0" step="1000" placeholder="Harga per malam">
                @error('price')
                    <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                @enderror
            </div>
