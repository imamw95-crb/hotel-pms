...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-xl font-bold text-gray-800">Tambah Kamar</h2>
            <p class="text-sm text-gray-500">Isi data kamar baru di bawah ini</p>
        </div>
    </div>

    <form method="POST" action="{{ route('rooms.store') }}" data-ajax="true" data-refresh="true">
        @csrf
        ...
        <div class="mb-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">No. Kamar <span class="text-red-500">*</span></label>
                    <input type="text" name="room_number" value="{{ old('room_number') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" placeholder="Contoh: 101, A-202" required>
                    @error('room_number')
                ...
                <div>
                    ...
                    <select name="room_type_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                        ...
                        @foreach($roomTypes as $type)
                            <option value="{{ $type->id }}" data-capacity="{{ $type->base_capacity ?? '' }}" {{ old('room_type_id') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                        @endforeach
                    </select>
                    @error('room_type_id')
                        <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="max_occupancy" value="{{ old('max_occupancy', 2) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required min="1" placeholder="Jumlah tamu maks.">
                    @error('max_occupancy')
                        <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <select name="status" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
                        ...
                        <option value="occupied" {{ old('status') === 'occupied' ? 'selected' : '' }}>Ditempati</option>
                        <option value="maintenance" {{ old('status') === 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                        <option value="cleaning" {{ old('status') === 'cleaning' ? 'selected' : '' }}>Pembersihan</option>
                    </select>
                    @error('status')
                        <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
