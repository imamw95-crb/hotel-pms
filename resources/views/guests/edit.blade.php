...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl mx-auto">
    ...
    <form method="POST" action="{{ route('guests.update', $guest->id) }}" data-ajax="true">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="guest_name" value="{{ old('guest_name', $guest->guest_name) }}" class="w-full border rounded px-3 py-2" required>
            @error('guest_name')
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="id_number" value="{{ old('id_number', $guest->id_number) }}" class="w-full border rounded px-3 py-2">
                @error('id_number')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div>
                ...
                <input type="tel" name="phone" value="{{ old('phone', $guest->phone) }}" class="w-full border rounded px-3 py-2">
                @error('phone')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="mb-4">
            ...
            <input type="email" name="email" value="{{ old('email', $guest->email) }}" class="w-full border rounded px-3 py-2">
            @error('email')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="notes" class="w-full border rounded px-3 py-2" rows="3">{{ old('notes', $guest->notes) }}</textarea>
            @error('notes')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
